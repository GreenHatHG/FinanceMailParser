...
#### 代码实现
- 所有需要多次复用的文本 / 数值 / 配置项，必须定义为语义化命名的常量，按照分层规则放到合适的位置。当需要使用某个文本 / 数值时，优先引用已定义的常量，而非重新编写相同内容，或者相似的内容，避免维护两份数据。
- 避免缺少抽象：没有接口/基类，相似功能重复实现。

#### 代码结构
- 避免缺乏分层：所有代码混在一起（业务逻辑、数据访问、配置、工具函数）。
- 避免耦合严重：模块间互相强依赖，改一处容易崩多处。
