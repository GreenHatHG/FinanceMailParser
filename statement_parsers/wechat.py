...
def parse_wechat_statement(file_path: str, start_date: Optional[datetime] = None, end_date: Optional[datetime] = None) -> List[Transaction]:
    # 微信账单格式：交易时间 交易类型 交易对方 商品 收/支 金额(元) 支付方式 当前状态 交易单号 商户单号 备注 共11列
    df = pd.read_csv(file_path, header=16, skipfooter=0, encoding='utf-8')  # 微信的对账单格式从第17行开始
    ...
    if filtered_keywords:
        logger.info(f"按关键字过滤掉 {len(filtered_keywords)} 条记录：{filtered_keywords}")
