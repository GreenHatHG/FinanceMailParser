...
# Streamlit executes page files via `exec`, and the default sys.path may not
# include the repo root when running `streamlit run ui/streamlit/app.py`.
# Add repo root so `import ui.*` works reliably in local dev.
...
ai_process_page = st.Page(
    str(PAGES_DIR / "ai" / "process_beancount.py"),
    title="AI 处理",
    icon="🤖",
)
