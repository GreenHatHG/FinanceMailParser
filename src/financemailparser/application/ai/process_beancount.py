...
from financemailparser.infrastructure.ai.service import AIService, CallStats
from financemailparser.application.ai.amount_masking import AmountMasker, MaskingStats
...
from financemailparser.infrastructure.beancount.file_manager import (
    ...
    scan_beancount_files as _scan_beancount_files,
)
...
def scan_beancount_files_for_ui(output_dir: Path) -> list[BeancountFileInfo]:
    """
    UI-facing helper to scan local Beancount output files.
    """
    return _scan_beancount_files(output_dir)


def read_beancount_file_for_ui(path: Path) -> Optional[str]:
    """
