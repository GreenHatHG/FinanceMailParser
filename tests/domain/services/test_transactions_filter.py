...
def test_partially_matches_multiple_transactions_in_same_group() -> None:
    ...
    txns = [
        ...
        _txn("2026-01-02", "Tea", 10.0),
        _txn("2026-01-05", "Refund", -10.0),
    ]
...
def test_matches_across_dates_same_source() -> None:
    ...
    txns = [
        _txn("2026-01-01", "Coffee", 10.0, TransactionSource.CCB),
        _txn("2026-01-10", "Refund", -10.0, TransactionSource.CCB),  # 9天后退款
    ]
    out = filter_matching_refunds(txns)
    assert out == []


def test_matches_same_source_different_descriptions() -> None:
    """同一来源、金额相等但描述不同的交易应该配对，只要退款日期晚于消费日期"""
    txns = [
        _txn("2026-01-01", "星巴克消费", 50.0, TransactionSource.CCB),
        _txn("2026-01-10", "退款", -50.0, TransactionSource.CCB),  # 10天后退款
    ]
    out = filter_matching_refunds(txns)
    assert out == []
